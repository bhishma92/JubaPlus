...
def main():
    ...
    df = pd.read_csv("hhmmss85.tsv", header=None, delimiter="\t", error_bad_lines = False)
    #df = pd.read_csv('/home/dummy/file.tsv', header=None, delimiter="\t", error_bad_lines = False)
                ...
                if (df[2][index-1] == df[2][index]):
                    ...
                    if sum_var - sum_var1 < 3600:
                        ...
                        if ses_day == df[2][index-1]:
                            ...
                            a = map(int, df.iloc[index-1, 2].split('-'))
                            b = map(int, df.iloc[ses_index, 2].split('-'))
                    ...
                    p = map(int, df.iloc[index, 2].split('-'))
                    q = map(int, df.iloc[index-1, 2].split('-'))

                    xx = datetime.datetime(p[2],p[0],p[1])
                    yy = datetime.datetime(q[2],q[0],q[1])
                    d = (xx - yy).days

                    if d ==1:
                        tt = sum_var - sum_var1 + 86400
                    else:
                        tt = 3600 
                    ...
                    if  tt < 3600:
                        ...
                        d2 = map(int, df.iloc[ses_index, 2].split('-'))
                        t1 = map(int, df.iloc[index-1, 3].strip(' GMT').split(':'))
                        t2 = map(int, df.iloc[ses_index, 3].strip(' GMT').split(':'))

                        a = datetime.datetime(q[2],q[0],q[1],t1[0],t1[1],t1[2])
    ...
    curr_time = df[3][len(df.index)-1].strip(' GMT').split(':')
    curr_time1 = df[3][ses_index].strip(' GMT').split(':')
    t1 = int(curr_time[0])*3600 + int(curr_time[1])*60 + int(curr_time[2])
    t2 = int(curr_time1[0])*3600 + int(curr_time1[1])*60 + int(curr_time1[2])
    duration = sessionTime(t2, t1)
    for i in range(ses_index, len(df.index)):
        arr2[i] = duration 
    ...
    #df.to_csv('/home/dummy/sesOut' + '.tsv', index = False, header= False, sep='\t')
