...
def main():
    ...
    prev = seconds.at[prev_row[0]]
    ses = seconds.at[ses_row[0]]
    
    for row in df.itertuples():
        try:
            g = 1/int(row[0])

            curr = seconds.at[row[0]]
                ...
                if prev_row[5] == row[5]:
                    if curr - prev < 3600:
                        ...
                        if ses_row[5] == prev_row[5]:
                            ...
                            d = (d1 - d2).days
                            s_var = prev + (d*86400)
                            duration = sessionTime(ses, s_var)
