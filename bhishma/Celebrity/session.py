...
def main():
    ...
    df = pd.read_csv('/home/dummy/try/fileNewFinal.tsv', header=None, delimiter="\t", error_bad_lines = False)
    #df = df.loc[df['C'] == 'US']     
    ...
    result = pd.concat([df, cs, cn], axis=1, join='inner')
        
    #df.to_csv("output.tsv", index = False, header= False, sep='\t')
    result.to_csv('/home/dummy/try/sesOutTry' + '.tsv', index = False, header= None, sep='\t')
